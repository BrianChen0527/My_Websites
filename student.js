...
var allAnimals = function getFavoriteAnimal(arr){
  return arr.map(student => student.favoriteAnimal);
}

allAnimals(students)
allAnimals(students).filter(an => an.length >= 5)

    var animalsArr = [];
    var animals = animalsArr.map(student => student.favoriteAnimal.split(''));

	var sortedFavoriteAnimals = animals.sort(function(a,b){
    return a - b;
    });

    animalsArr.push(sortedFavoriteAnimals);
    var finalSortedFavAnimals = sortedFavoriteAnimals.map(animals => animals.join('-'));
    return finalSortedFavAnimals
}


var x=[25, 8, 7, 41]
x.sort(function(a,b){ //Array now becomes [7, 8, 25, 41]
    return a - b
})


    sortFavoriteAnimal(animalsArr);
